...
function Progress() {
  ...
  const handleAddCard = () => {
    ...
    localStorage.setItem("progressList", JSON.stringify(cardValues));
  };
  ...
  const handleRemoveCard = (index) => {
    ...
    localStorage.setItem("progressList", JSON.stringify(updatedCardValues));
  };

  useEffect(() => {
    const savedValues = localStorage.getItem("progressList");
    if (savedValues) {
      ...
      <div className="flex flex-col">
        <div className="bg-gray-200 rounded h-auto w-96 ml-2 flex flex-col">
          <div className="ml-4 pt-3">
            <h1 className="text-black font-bold text-lg">In Progress</h1>
          </div>
          {Array(cardCount)
            .fill(null)
            .map((_, index) => (
